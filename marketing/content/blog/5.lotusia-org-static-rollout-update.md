...
- Added image `width`/`height` where possible to reduce [layout shift (CLS)](https://web.dev/articles/cls).
- Improved [`hreflang`](https://developers.google.com/search/docs/specialty/international/localized-versions) generation to avoid duplicate alternate links pointing to the same URL on non-translated pages.
- Added [sitemap](https://www.sitemaps.org/protocol.html) `lastmod` support based on content dates and file modification times.
...
Explore the live results: browse [social profiles](/social/profiles) powered by on-chain reputation, or read about [how the ranking algorithm works](/blog/why-we-built-our-own-ranking-algorithm).
