...
This article describes how [we](/founders) built truly decentralized multi-signature coordination for Lotus, and why it matters for the broader cryptocurrency ecosystem.

## What is MuSig2 and Why Does It Matter?

MuSig2 is a [multi-signature](https://en.wikipedia.org/wiki/Multisignature) protocol for [Schnorr signatures](https://en.wikipedia.org/wiki/Schnorr_signature). Multiple parties collaborate to produce a single, aggregated signature that's cryptographically indistinguishable from a single-signature transaction.

The privacy and efficiency gains are substantial. Traditional [P2SH](https://en.bitcoin.it/wiki/Pay_to_script_hash) multi-signature for three participants requires 99 bytes of public keys and 210 bytes of signatures—309 bytes total. MuSig2 with Taproot uses 33 bytes for the public key and 64 bytes for the signature—97 bytes, a 69% reduction. More importantly, the transaction provides no information about how many parties were involved or what the signing threshold was.
...
When combined with [Taproot](https://en.bitcoin.it/wiki/Taproot), MuSig2 enables sophisticated on-chain behaviors that look like basic payments:
...
Each participant generates nonces using [RFC 6979](https://www.rfc-editor.org/rfc/rfc6979) deterministic generation (with additional random entropy by default for production security) and shares the public nonces. Everyone must receive everyone else's public nonces before proceeding.
...
We use [libp2p](https://libp2p.io/)'s proven infrastructure with three complementary discovery mechanisms:

**DHT ([Distributed Hash Table](https://en.wikipedia.org/wiki/Distributed_hash_table))**: Session announcements are stored in the [Kademlia](https://en.wikipedia.org/wiki/Kademlia) DHT (with k=20 replication) using a session identifier derived from the participating signers and message. When Alice creates a signing session, she publishes an announcement to the DHT. Bob and Charlie can discover it by querying the DHT with the session ID. This provides persistent, decentralized storage without a central server—sessions remain discoverable even when the creator goes offline.

**[GossipSub](https://docs.libp2p.io/concepts/pubsub/)**: For real-time signer discovery, participants publish advertisements to PubSub topics organized by transaction type (e.g., `musig2:signers:SWAP`). When Bob advertises availability for atomic swaps, Alice receives the notification in 10-100ms if she's subscribed to that topic. This provides instant peer discovery without DHT polling delays.

**Direct Message Protocol**: Once participants discover each other, coordination messages (nonces, partial signatures) are sent via libp2p's message protocol with peer-to-peer routing. Messages are encrypted using libp2p's [Noise protocol](https://noiseprotocol.org/) and routed through the network to specific peers rather than broadcast globally.
...
**SwapSig Privacy Protocol**: Our [CoinJoin](https://en.bitcoin.it/wiki/CoinJoin)-equivalent where multiple parties coordinate to create a single transaction with merged inputs and outputs, breaking on-chain traceability while maintaining individual signature verification. A future blog article will do a deep-dive analysis on SwapSig.
...
This implementation is cryptocurrency-agnostic. The coordination layer operates independently of [blockchain consensus](https://en.wikipedia.org/wiki/Blockchain#Consensus)—pure P2P networking and cryptographic message passing. [Bitcoin](https://en.wikipedia.org/wiki/Bitcoin) can adopt it for Taproot multi-sig. [Ethereum](https://en.wikipedia.org/wiki/Ethereum) can use it for [threshold signatures](https://en.wikipedia.org/wiki/Threshold_cryptosystem) in [account abstraction](https://eips.ethereum.org/EIPS/eip-4337). Any cryptocurrency supporting Schnorr signatures can integrate this infrastructure. Lightning channels, atomic swaps, privacy protocols, DAO governance, and DeFi operations all require interactive multi-party coordination. The same infrastructure solves all these problems.
...
Now comes the critical part: experimentation and iteration. Software matures through genuine use, not speculation. See how community reputation works in practice on [Lotusia social profiles](/social/profiles), or read about [how we rank content](/blog/why-we-built-our-own-ranking-algorithm). Install lotus-sdk, experiment with the examples, break the protocol, report issues. Real-world usage will uncover edge cases and inform the next iteration. Lotus has always been about doing things properly. The Lotusian Turtle moves slowly, but deliberately, towards victory; one careful step at a time.
