...
function normalizeLocaleTag(value) {
  const raw = String(value || 'en-US').trim();
  if (!raw) return 'en-US';
  return raw.replace(/_/g, '-');
}

let __workerFormatLang = 'en';

function setWorkerFormatLang(lang) {
  if (WORKER_LANGS.includes(lang)) {
    __workerFormatLang = lang;
  } else {
    __workerFormatLang = 'en';
  }
}

function localeFromLang(langOrLocale) {
  const candidate = String(langOrLocale || '').trim();
  if (!candidate) {
    return normalizeLocaleTag(workerI18nValue(__workerFormatLang, 'locale', 'en_US'));
  }
  if (WORKER_LANGS.includes(candidate)) {
    return normalizeLocaleTag(workerI18nValue(candidate, 'locale', 'en_US'));
  }
  return normalizeLocaleTag(candidate || 'en-US');
}

const __intlCache = new Map();

function numberFormatter(langOrLocale, options) {
  const locale = localeFromLang(langOrLocale);
  const key = locale + '|' + JSON.stringify(options || {});
  if (__intlCache.has(key)) return __intlCache.get(key);
  const formatter = new Intl.NumberFormat(locale, options || {});
  __intlCache.set(key, formatter);
  return formatter;
}

function formatUtc(value, langOrLocale) {
  const n = Number(value || 0);
  if (!Number.isFinite(n) || n <= 0) return '-';
  const d = new Date((String(value).length > 11 ? n : n * 1000));
  if (Number.isNaN(d.getTime())) return '-';
  return d.toLocaleString(localeFromLang(langOrLocale), {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
    hour: 'numeric',
...
function formatXpiFromSats(sats, langOrLocale) {
  const n = Number(sats || 0);
  if (!Number.isFinite(n)) return '0 XPI';
  const xpi = n / 1000000;
  const text = numberFormatter(langOrLocale, {
    minimumFractionDigits: Number.isInteger(xpi) ? 0 : 2,
    maximumFractionDigits: 2
  }).format(xpi);
  return text + ' XPI';
}

function voteRatio(positive, negative, langOrLocale) {
  const p = num(positive);
  const n = num(negative);
  const total = p + n;
  if (total <= 0) return '0%';
  return numberFormatter(langOrLocale, {
    style: 'percent',
...
function voteRatioPill(positive, negative, langOrLocale) {
  const ratioText = voteRatio(positive, negative, langOrLocale);
  const p = num(positive);
  const n = num(negative);
  const total = p + n;
  const ratioValue = total > 0 ? (p / total) * 100 : 0;
...
function voteToneHtml(sentiment, sats, langOrLocale) {
  if (sentiment === 'positive') return '<span class="flex items-center gap-1 text-green-500 dark:text-green-400">' + iconSvg('up', 'h-4 w-4') + esc(formatXpiFromSats(sats, langOrLocale)) + '</span>';
  if (sentiment === 'negative') return '<span class="flex items-center gap-1 text-red-500 dark:text-red-400">' + iconSvg('down', 'h-4 w-4') + esc(formatXpiFromSats(sats, langOrLocale)) + '</span>';
...
function paginationHtml(basePath, page, pageSize, numPages, options) {
  ...
  const fmt = function(v) { return formatNumber(v, lang); };
  ...
  const pageSizeItems = pageSizes.map(function(size) {
    if (size === pageSize) {
      return '<span class="block px-3 py-2 text-sm font-semibold rounded-lg bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-200 tabular-nums">' + fmt(size) + '</span>';
    }
    return '<a href="' + mk(1, size) + '" class="block px-3 py-2 text-sm rounded-lg text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 tabular-nums">' + fmt(size) + '</a>';
  }).join('');
  const selectControl = '<div class="relative w-28" data-page-size-root>' +
    ...
    '<span>' + fmt(pageSize) + '</span><span class="text-xs text-gray-400">▾</span></button>' +
  ...
  for (let p = windowStart; p <= windowEnd; p += 1) {
    if (p === safePage) pageButtons.push('<span class="inline-flex items-center justify-center w-[4.4rem] h-11 px-2 text-sm rounded-xl bg-primary-500 text-white dark:bg-primary-400 dark:text-gray-900 font-bold shadow-sm ring-2 ring-primary-500/25 tabular-nums whitespace-nowrap">' + fmt(p) + '</span>');
    else pageButtons.push('<a href="' + mk(p, pageSize) + '" class="inline-flex items-center justify-center w-[4.4rem] h-11 px-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 hover:border-primary-300 dark:hover:border-primary-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors tabular-nums whitespace-nowrap">' + fmt(p) + '</a>');
  }
  const endCaps = (windowStart > 1 ? '<a href="' + mk(1, pageSize) + '" class="inline-flex items-center justify-center w-[4.4rem] h-11 px-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors tabular-nums whitespace-nowrap">' + fmt(1) + '</a><span class="px-1 text-gray-400 whitespace-nowrap">…</span>' : '') +
    pageButtons.join('') +
    (windowEnd < totalPages ? '<span class="px-1 text-gray-400 whitespace-nowrap">…</span><a href="' + mk(totalPages, pageSize) + '" class="inline-flex items-center justify-center w-[4.4rem] h-11 px-2 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors tabular-nums whitespace-nowrap">' + fmt(totalPages) + '</a>' : '');
  ...
  return '<div class="mt-0 rounded-b-2xl border border-gray-200/90 dark:border-gray-700/70 border-t-0 bg-gray-50/70 dark:bg-gray-900/60 px-4 py-3 overflow-visible"' + groupAttr + '>' +
    ...
    '<div class="flex flex-wrap items-center gap-2 sm:gap-3"><span class="inline-flex items-center justify-center h-11 px-3 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 rounded-xl tabular-nums whitespace-nowrap">' + esc(pageLabel) + ' ' + fmt(safePage) + ' ' + esc(ofLabel) + ' ' + fmt(totalPages) + '</span><div class="flex flex-wrap items-center gap-2 sm:whitespace-nowrap">' + prevLink + endCaps + nextLink + '</div></div>' +
    '</div>' +
    '</div>';
}

function formatNumber(value, langOrLocale) {
  const n = Number(value || 0);
  if (!Number.isFinite(n)) return '0';
  return numberFormatter(langOrLocale).format(n);
}

function formatBytes(value, langOrLocale) {
  const n = Number(value || 0);
  if (!Number.isFinite(n)) return '-';
  return formatNumber(n, langOrLocale) + ' B';
}

function formatPercent(value, fractionDigits, langOrLocale) {
  const n = Number(value || 0);
  if (!Number.isFinite(n)) return '0%';
  const digits = Number.isFinite(Number(fractionDigits)) ? Math.max(0, Math.floor(Number(fractionDigits))) : 2;
  return numberFormatter(langOrLocale, {
